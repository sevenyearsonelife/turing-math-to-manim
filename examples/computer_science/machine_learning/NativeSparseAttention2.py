...
# Configuration for camera and resolution
config.frame_width = 20  # Adjust these values
config.frame_height = 11.25
config.pixel_width = 1920  # Adjust resolution if necessary
...
class Scene1_Introduction(Scene):
    def construct(self):
        # Title text
        title = Text("Vanilla Self-Attention", font_size=40)
        title.to_edge(UP)
        # Attention formula with colored Q, K, V
        formula = MathTex(
            r"\text{Attention}(Q, K, V) = \text{softmax}\!\Big(\frac{QK^T}{\sqrt{d_k}}\Big)\,V",
            font_size=30,
        )
        ...
        tokens = (
            VGroup(*[Dot(radius=0.08) for _ in range(6)]).arrange(RIGHT, buff=0.6).scale(0.8)
        )
        ...
        for j, token in enumerate(tokens):
            if token != query_token:
                ...
                arrows.add(arrow)
        # Complexity text
        complexity = Text("Interactions: $n \\times n$ (quadratic)", font_size=24)
...
class Scene2_QKVBreakdown(Scene):
    def construct(self):
        ...
        token_label = Text("Token", font_size=24).next_to(token, DOWN, buff=0.2)
        ...
        # Labels for Q, K, V
        Q_label = Text("Q (query)", color=BLUE, font_size=22).next_to(Q_vec, UP * 0.5)
        K_label = Text("K (key)", color=RED, font_size=22).next_to(K_vec, DOWN * 0.5)
        V_label = Text("V (value)", color=GREEN, font_size=22).next_to(V_vec, DOWN * 0.5)
        ...
        other_keys = (
            VGroup(*[Circle(radius=0.1, color=RED, fill_opacity=1) for _ in range(3)])
            .arrange(DOWN, buff=0.4)
            .scale(0.8)
        )
        other_vals = (
            VGroup(*[Circle(radius=0.1, color=GREEN, fill_opacity=1) for _ in range(3)])
            .arrange(DOWN, buff=0.4)
            .scale(0.8)
        )
        other_kv = (
            VGroup(other_keys, other_vals).arrange(RIGHT, buff=1).to_edge(RIGHT, buff=2)
        )
        # Labels for other tokens' keys and values
        ok_label = Text("Other Keys", font_size=20, color=RED).next_to(
            other_keys, UP, buff=0.2
        )
        ov_label = Text("Other Values", font_size=20, color=GREEN).next_to(
            other_vals, UP, buff=0.2
        )
        ...
        # Output circle for weighted sum
        output_circle = Circle(radius=0.12, color=GREEN, fill_opacity=0.8).to_edge(
            RIGHT, buff=0.5
        )
        output_label = Text("Weighted\nsum of\nValues", font_size=18).next_to(
            output_circle, RIGHT
        )
        ...
        # Calculate dot products and softmax weights
        raw_scores = []
        for ok in other_keys:
            q_vec_np = Q_vec.get_end() - Q_vec.get_start()
            k_vec_np = ok.get_center() - Q_vec.get_end()
            q_vec_np = q_vec_np / np.linalg.norm(q_vec_np)
            k_vec_np = k_vec_np / np.linalg.norm(k_vec_np)
            dot_product = np.dot(q_vec_np[:2], k_vec_np[:2])
            raw_scores.append(dot_product)
        ...
        # Display softmaxed weights and animate arrows
        for i, conn in enumerate(connectors):
            weight_text = MathTex(
                rf"w = {softmax_weights[i]:.2f}", font_size=24
            ).next_to(conn, UP, buff=0.1)
        ...
        for i, val in enumerate(other_vals):
            scaled_val = Circle(
                radius=0.12 * softmax_weights[i],
                color=GREEN,
                fill_opacity=0.8 * softmax_weights[i],
            )
            scaled_val.move_to(val)  # Position at original value
            output_circles.add(scaled_val)
            self.play(Transform(val, scaled_val))  # Transform original to scaled

        # Then animate the arrows to the output_circle, showing the weighted sum:
        for i, val in enumerate(other_vals):
            apply_arrow = Arrow(
                val.get_right(),
                output_circle.get_left(),
                buff=0.1,
                stroke_width=4 * softmax_weights[i],
                color=GREEN,
                tip_length=0.15,
            )
        ...
        self.play(
            *[
            ...
            + [FadeOut(mob) for mob in self.mobjects if isinstance(mob, MathTex)]
            + [FadeOut(mob) for mob in output_circles]
...
class Scene3_ComplexityProblem(Scene):
    def construct(self):
        ...
        problem_text = Text("Long Sequence = Explosive Attention Cost", font_size=32)
        problem_text.to_edge(UP)
        # Example: n = 1000, interactions = 1,000,000
        example_text = Text(
            "e.g.,  n = 1000 tokens  ->  1000 x 1000 = 1,000,000 interactions",
            font_size=24,
        )
        example_text.next_to(problem_text, DOWN, buff=0.5)
        example_text.set_color(RED)
        # Represent attention matrix growth (small -> large)
        small_matrix = Integer(16)  # just showing a number of interactions
        small_matrix_text = Text("4x4 = 16", font_size=20).next_to(
            small_matrix, DOWN, buff=0.1
        )
        small_matrix_group = VGroup(small_matrix, small_matrix_text).move_to(
            LEFT * 3 + DOWN * 1
        )
        big_matrix = Integer(1000000)
        big_matrix_text = Text("1000x1000 = 1,000,000", font_size=20).next_to(
            big_matrix, DOWN, buff=0.1
        )
        big_matrix_group = VGroup(big_matrix, big_matrix_text).move_to(
            RIGHT * 3 + DOWN * 1
        )
        # Arrows pointing from small to big to show growth
        arrow_growth = Arrow(
            small_matrix_group.get_right(),
            big_matrix_group.get_left(),
            buff=0.5,
            color=YELLOW,
            tip_length=0.15,
        )
        growth_label = Text("n\u2191 => n^2\u2191\u2191", font_size=24, color=YELLOW).next_to(
            arrow_growth, UP
        )
        ...
        self.play(
            ...
            FadeOut(small_matrix_group),
            FadeOut(big_matrix_group),
            FadeOut(arrow_growth),
...
class Scene4_OptimizedOverview(Scene):
    def construct(self):
        title = Text("Optimized Attention: Key Ideas", font_size=36)
        title.to_edge(UP)
        idea1 = Text("1. Sliding Window (local neighbors)", font_size=26, color=BLUE)
        idea2 = Text("2. Compression (summarize segments)", font_size=26, color=RED)
        idea3 = Text("3. Selection (important tokens)", font_size=26, color=YELLOW)
        ideas = (
            VGroup(idea1, idea2, idea3)
        ...
        sub1 = Text(
            "Limit attention to a fixed window [linear complexity]",
            font_size=18,
            color=BLUE,
        ).next_to(idea1, RIGHT, buff=0.3)
        sub2 = Text(
            "Compress long-range context [LongT5 blocks]", font_size=18, color=RED
        ).next_to(idea2, RIGHT, buff=0.3)
        sub3 = Text(
            "Attend to key tokens globally [Longformer]", font_size=18, color=YELLOW
        ).next_to(idea3, RIGHT, buff=0.3)
...
class Scene5_SlidingWindow(Scene):
    def construct(self):
        title = Text("Sliding Window Attention (Local)", font_size=32, color=BLUE).to_edge(
            UP
        )
        # Create a row of tokens (e.g., 12 tokens)
        n = 12
        tokens = (
            VGroup(*[Square(side_length=0.3, fill_opacity=1) for _ in range(n)])
        ...
        window_indices = list(
            range(max(0, query_index - w), min(n, query_index + w + 1))
        )
        ...
        window_rect = SurroundingRectangle(
            VGroup(*tokens[left_idx : right_idx + 1]), color=BLUE, buff=0.05
        )
        ...
        complexity_note = Text(
            "Local attention: O(n \u00d7 w)", font_size=24, color=BLUE
        )
        ...
        # Continuous sliding:
        for new_index in range(w, n - w):  # Iterate through possible query positions
            new_window_indices = list(range(new_index - w, new_index + w + 1))
            new_left, new_right = new_window_indices[0], new_window_indices[-1]
            new_rect = SurroundingRectangle(
                VGroup(*tokens[new_left : new_right + 1]),
                color=BLUE,
                buff=0.05,
                fill_opacity=0.1,
            )
            new_query_marker = SurroundingRectangle(
                tokens[new_index], color=BLUE, buff=0.0, fill_opacity=0.2
            )
            ...
            self.play(
                window_rect.animate.become(new_rect),
                query_marker.animate.become(new_query_marker),
                *[FadeOut(arr) for arr in arrows],
                *[GrowArrow(na) for na in new_arrows],
                run_time=0.5,  # Adjust for speed
            )
            arrows = new_arrows
...
class Scene6_Compression(Scene):
    def construct(self):
        title = Text(
            "Compression: Summarize Distant Tokens", font_size=32, color=RED
        ).to_edge(UP)
        ...
        for b in range(num_blocks):
            ...
            brace_text = Text("Compress", font_size=20, color=RED).next_to(
                brace, UP, buff=0.1
            )
            braces.add(brace)
            ...
            # self.play(braces[b].animate.set_color(RED), FadeIn(brace_texts[b])) #Removed to reduce clutter

            # Squishing animation:
            self.play(
                *[token.animate.move_to(summary.get_center()) for token in block_group],
                *[
                    FadeToColor(token, RED) for token in block_group
                ],  # Fade to summary color
                GrowFromCenter(summary),
                FadeOut(brace),
                FadeOut(brace_text),
                run_time=0.8,
            )
            self.play(*[FadeOut(token) for token in block_group])  # Fade out originals
            summary_tokens.add(summary)

        # All summary tokens now represent the blocks
        summary_label = Text("Summaries", font_size=24, color=RED).next_to(
            summary_tokens, UP, buff=0.5
        )
        self.play(FadeIn(summary_label))
        self.wait(1)
        # Fade out original tokens and keep summaries (to imply compression of detail)
        # self.play(FadeOut(tokens), FadeOut(title)) #Removed fadeout of tokens.
...
class Scene7_Selection(Scene):
    def construct(self):
        title = Text(
            "Selection: Important Global Tokens", font_size=32, color=YELLOW
        ).to_edge(UP)
        ...
        important_indices = [
            ...
            6,
            10,
        ]  # Example indices of important tokens (within range 0-11)
        ...
        label = Text("Globally important tokens", font_size=24, color=YELLOW)
        ...
        for i, glow in enumerate(important_marks):
            ...
            targets = [0, n - 1] if token_idx not in [
                0,
                n - 1,
            ] else [
                n // 2
            ]  # choose endpoints or middle
            for t in targets:
                if t == token_idx:
                    continue
                arr = Arrow(
        ...
        self.play(
            FadeOut(title),
            FadeOut(tokens),
            FadeOut(important_marks),
            FadeOut(label),
            FadeOut(broadcast_arrows),
        )
...
class Scene8_CombineLocalGlobal(Scene):
    def construct(self):
        title = Text("Combined Local + Global Attention", font_size=32).to_edge(UP)
        # Use a smaller sequence for clarity, say 8 tokens
        n = 8
        tokens = (
            VGroup(*[Circle(radius=0.15, fill_opacity=1) for _ in range(n)])
        ...
        query_label = Text("Query", font_size=20, color=BLUE).next_to(
            query_token, DOWN, buff=0.1
        )
        # Define local window range (e.g., 1 on each side)
        w = 1
        local_neighbors = [
            i
            for i in range(max(0, query_idx - w), min(n, query_idx + w + 1))
            if i != query_idx
        ]
        # Mark local neighbors by cyan outline
        local_highlights = VGroup(
            *[
                SurroundingRectangle(tokens[i], color=BLUE, buff=0.02).set_stroke(
                    width=3
                )
        ...
        for gi in global_indices:
            ...
            glow = SurroundingRectangle(
                tokens[gi], color=YELLOW, buff=0.05
            ).set_stroke(width=4, color=YELLOW)
            global_highlights.add(glow)
        global_label = Text("Global info", font_size=20, color=YELLOW)
        ...
        combine_text = Text("Combine local + global attention", font_size=24)
        ...
        self.play(
            *[GrowArrow(arr) for arr in local_arrows],
            *[GrowArrow(arr) for arr in global_arrows],
        )
        ...
        self.play(
            ...
            FadeOut(tokens),
            FadeOut(query_label),
            FadeOut(local_highlights),
            FadeOut(global_highlights),
            FadeOut(global_label),
            FadeOut(local_arrows),
            FadeOut(global_arrows),
...
class Scene9_EfficientPattern(Scene):
    def construct(self):
        title = Text("Result: Sparse Attention Pattern", font_size=32).to_edge(UP)
        ...
        for i in range(n):
            for j in range(n):
                square = Square(
                    side_length=size, stroke_width=1, stroke_color=GRAY, fill_opacity=0
                )
        ...
        for i in range(n):
            for j in range(n):
                ...
                if abs(i - j) <= w:
                    ...
                    band_sq.move_to(
                        np.array([(j - (n - 1) / 2) * size, -(i - (n - 1) / 2) * size, 0])
                    )
        ...
        complexity_text = Text(
            "Complexity ~ O(n * (w + g))", font_size=24, color=GREEN
        )
        complexity_text.to_edge(DOWN)
        note_text = Text(
            "Most of the matrix is empty (no computation)", font_size=20
        ).next_to(complexity_text, UP)
        ...
        self.play(
            Indicate(band_group, scale_factor=1.05),
            Indicate(global_group, scale_factor=1.05),
        )
        self.wait(1)
        # Fade out everything
        self.play(
...
class Scene10_Conclusion(Scene):
    def construct(self):
        # Recap bullet points
        recap1 = Text(
            "Local sliding window (blue): linear-time local context",
            font_size=22,
            color=BLUE,
        )
        recap2 = Text(
            "Compressed summaries (red): fewer tokens to attend",
            font_size=22,
            color=RED,
        )
        recap3 = Text(
            "Selected key tokens (yellow): preserve global info",
            font_size=22,
            color=YELLOW,
        )
        recaps = (
            VGroup(recap1, recap2, recap3)
        ...
        final_statement = Text(
            "Result: Efficient attention (near-linear) with long-range coverage",
            font_size=24,
            color=WHITE,
        )
